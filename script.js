...
const getWeather = () => {
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      // Clears all current statuses and updates all current day statuses with icon
      $('#stats').remove();
      ...
      // Checks localStorage for previously entered locations. If not already in localStorage, sets location in localStorage. Makes each location a clicable button that runs getWeather function again for that location
      ...
      // Clears search box after search is submitted
      $searchInput.val('')
};

// Runs the function on click, Enter and click on history button
